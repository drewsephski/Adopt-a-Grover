        ...
        <Table>
            ...
            <TableBody>
                {initialCampaigns.map((campaign) => {
                    ...
                    return (
                        <TableRow key={campaign.id} className="group transition-colors hover:bg-muted/50">
...
function StatusBadge({ status }: { status: CampaignWithFamilies['status'] }) {
    const styles: Record<string, string> = {
        [CAMPAIGN_STATUS.DRAFT]: "bg-muted text-muted-foreground border-border",
        [CAMPAIGN_STATUS.ACTIVE]: "bg-primary/10 text-primary border-primary/20",
        [CAMPAIGN_STATUS.CLOSED]: "bg-secondary text-secondary-foreground border-border",
        [CAMPAIGN_STATUS.ARCHIVED]: "bg-destructive/10 text-destructive border-destructive/20",
    };
