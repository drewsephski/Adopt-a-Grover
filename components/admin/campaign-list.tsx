...
import { ChevronRight, Target, Users, Package } from "lucide-react";
...
export function CampaignList({ initialCampaigns }: CampaignListProps) {
        ...
        return (
            <div className="flex flex-col items-center justify-center py-16 sm:py-20 text-center px-4">
                ...
                <p className="text-muted-foreground text-sm max-w-sm">
                    Create your first campaign to start organizing donation drives.
                </p>
            </div>
        );
    }
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case "ACTIVE":
                return "bg-green-500/15 text-green-700 dark:text-green-400 border-green-300/50 dark:border-green-700/50";
            case "DRAFT":
                return "bg-gray-500/15 text-gray-700 dark:text-gray-400 border-gray-300/50 dark:border-gray-700/50";
            case "CLOSED":
                return "bg-blue-500/15 text-blue-700 dark:text-blue-400 border-blue-300/50 dark:border-blue-700/50";
            case "ARCHIVED":
                return "bg-orange-500/15 text-orange-700 dark:text-orange-400 border-orange-300/50 dark:border-orange-700/50";
            default:
                return "bg-gray-500/15 text-gray-700 dark:text-gray-400 border-gray-300/50 dark:border-gray-700/50";
        }
    };
        ...
        <>
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="bg-muted/50 hover:bg-muted/50">
                            ...
                            <TableHead className="font-semibold">Families</TableHead>
                    ...
                    <TableBody>
                        {initialCampaigns.map((campaign) => {
                            ...
                            return (
                                <TableRow key={campaign.id} className="group transition-colors hover:bg-muted/30">
            ...
            <div className="md:hidden space-y-4">
                {initialCampaigns.map((campaign) => {
                    ...
                    return (
                        <Card key={campaign.id} className="overflow-hidden hover:shadow-md transition-all duration-200 border-border/50">
                            <Link
                                href={`/admin/campaigns/${campaign.id}`}
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
                                    <div className="flex items-center gap-1">
                                        ...
                                        <span>{stats.totalFamilies} {stats.totalFamilies === 1 ? 'family' : 'families'}</span>
                                    </div>
                                    <span className="text-muted-foreground/50">•</span>
                                    <div className="flex items-center gap-1">
                                        <Package className="h-3.5 w-3.5 flex-shrink-0" />
                                        <span>{stats.totalGifts} {stats.totalGifts === 1 ? 'gift' : 'gifts'}</span>
                                    </div>
                                </div>
